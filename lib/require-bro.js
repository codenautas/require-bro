...
(function(){
    ...
    if(!window.define){
        window.define = function define(){
            ...
            if(argPos<arguments.length && typeof arguments[argPos] === "string"){
                ...
                name=window.globalModuleName || document.currentScript.src.replace(/.*\/([^/\.]*)((\.umd)?\.min)?(.js(\?[0-9a-zA-Z]+)?)?/,function(all,part){ return part; });
            }
            if(window.requireBroLogAll){
                window.requireBroLogAll.textContent += '\ndefine '+name;
            }
    ...
    window.requireBro.bootstrap = function bootstrap(esmModules) {
        esmNeeded = true;
        var batchPromise = Promise.all(esmModules.map(async function(name){
            var mod = await import(name);
            window.requireBro.definedModules[name] = mod;
        }));
        queuePromise = queuePromise.then(function(){ return batchPromise; });
        return queuePromise;
    };
