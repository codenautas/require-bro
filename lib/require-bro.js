...
(function(){
    ...
    window.requireBro.bootstrap = function bootstrap(esmModules) {
        esmNeeded = true;
        var batchPromise = Promise.all(esmModules.map(async function(spec){
            var name = typeof spec === 'string' ? spec : spec.name;
            var importTarget = typeof spec === 'string'
                ? spec
                : new URL(spec.url, document.baseURI).href;
            var mod = await import(importTarget);
